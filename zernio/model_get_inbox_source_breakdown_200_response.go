/*
Zernio API

API reference for Zernio. Authenticate with a Bearer API key. Base URL: https://zernio.com/api  Versioning and deprecation: all endpoints are versioned in the URL path (current version: /v1). Breaking changes only ship in a new path version; existing versions keep working. Deprecated operations are marked 'deprecated: true' in this spec and announced in the changelog (https://zernio.com/changelog) before removal.  Errors: every 4xx/5xx response is application/json with a machine-readable 'code' and a human-readable 'error' message (see the ErrorResponse schema).

API version: 1.0.4
Contact: support@zernio.com
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package zernio

import (
	"encoding/json"
)

// checks if the GetInboxSourceBreakdown200Response type satisfies the MappedNullable interface at compile time
var _ MappedNullable = &GetInboxSourceBreakdown200Response{}

// GetInboxSourceBreakdown200Response struct for GetInboxSourceBreakdown200Response
type GetInboxSourceBreakdown200Response struct {
	Success *bool                                            `json:"success,omitempty"`
	From    *string                                          `json:"from,omitempty"`
	To      NullableString                                   `json:"to,omitempty"`
	Sources []GetInboxSourceBreakdown200ResponseSourcesInner `json:"sources,omitempty"`
}

// NewGetInboxSourceBreakdown200Response instantiates a new GetInboxSourceBreakdown200Response object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewGetInboxSourceBreakdown200Response() *GetInboxSourceBreakdown200Response {
	this := GetInboxSourceBreakdown200Response{}
	return &this
}

// NewGetInboxSourceBreakdown200ResponseWithDefaults instantiates a new GetInboxSourceBreakdown200Response object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewGetInboxSourceBreakdown200ResponseWithDefaults() *GetInboxSourceBreakdown200Response {
	this := GetInboxSourceBreakdown200Response{}
	return &this
}

// GetSuccess returns the Success field value if set, zero value otherwise.
func (o *GetInboxSourceBreakdown200Response) GetSuccess() bool {
	if o == nil || IsNil(o.Success) {
		var ret bool
		return ret
	}
	return *o.Success
}

// GetSuccessOk returns a tuple with the Success field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *GetInboxSourceBreakdown200Response) GetSuccessOk() (*bool, bool) {
	if o == nil || IsNil(o.Success) {
		return nil, false
	}
	return o.Success, true
}

// HasSuccess returns a boolean if a field has been set.
func (o *GetInboxSourceBreakdown200Response) HasSuccess() bool {
	if o != nil && !IsNil(o.Success) {
		return true
	}

	return false
}

// SetSuccess gets a reference to the given bool and assigns it to the Success field.
func (o *GetInboxSourceBreakdown200Response) SetSuccess(v bool) {
	o.Success = &v
}

// GetFrom returns the From field value if set, zero value otherwise.
func (o *GetInboxSourceBreakdown200Response) GetFrom() string {
	if o == nil || IsNil(o.From) {
		var ret string
		return ret
	}
	return *o.From
}

// GetFromOk returns a tuple with the From field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *GetInboxSourceBreakdown200Response) GetFromOk() (*string, bool) {
	if o == nil || IsNil(o.From) {
		return nil, false
	}
	return o.From, true
}

// HasFrom returns a boolean if a field has been set.
func (o *GetInboxSourceBreakdown200Response) HasFrom() bool {
	if o != nil && !IsNil(o.From) {
		return true
	}

	return false
}

// SetFrom gets a reference to the given string and assigns it to the From field.
func (o *GetInboxSourceBreakdown200Response) SetFrom(v string) {
	o.From = &v
}

// GetTo returns the To field value if set, zero value otherwise (both if not set or set to explicit null).
func (o *GetInboxSourceBreakdown200Response) GetTo() string {
	if o == nil || IsNil(o.To.Get()) {
		var ret string
		return ret
	}
	return *o.To.Get()
}

// GetToOk returns a tuple with the To field value if set, nil otherwise
// and a boolean to check if the value has been set.
// NOTE: If the value is an explicit nil, `nil, true` will be returned
func (o *GetInboxSourceBreakdown200Response) GetToOk() (*string, bool) {
	if o == nil {
		return nil, false
	}
	return o.To.Get(), o.To.IsSet()
}

// HasTo returns a boolean if a field has been set.
func (o *GetInboxSourceBreakdown200Response) HasTo() bool {
	if o != nil && o.To.IsSet() {
		return true
	}

	return false
}

// SetTo gets a reference to the given NullableString and assigns it to the To field.
func (o *GetInboxSourceBreakdown200Response) SetTo(v string) {
	o.To.Set(&v)
}

// SetToNil sets the value for To to be an explicit nil
func (o *GetInboxSourceBreakdown200Response) SetToNil() {
	o.To.Set(nil)
}

// UnsetTo ensures that no value is present for To, not even an explicit nil
func (o *GetInboxSourceBreakdown200Response) UnsetTo() {
	o.To.Unset()
}

// GetSources returns the Sources field value if set, zero value otherwise.
func (o *GetInboxSourceBreakdown200Response) GetSources() []GetInboxSourceBreakdown200ResponseSourcesInner {
	if o == nil || IsNil(o.Sources) {
		var ret []GetInboxSourceBreakdown200ResponseSourcesInner
		return ret
	}
	return o.Sources
}

// GetSourcesOk returns a tuple with the Sources field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *GetInboxSourceBreakdown200Response) GetSourcesOk() ([]GetInboxSourceBreakdown200ResponseSourcesInner, bool) {
	if o == nil || IsNil(o.Sources) {
		return nil, false
	}
	return o.Sources, true
}

// HasSources returns a boolean if a field has been set.
func (o *GetInboxSourceBreakdown200Response) HasSources() bool {
	if o != nil && !IsNil(o.Sources) {
		return true
	}

	return false
}

// SetSources gets a reference to the given []GetInboxSourceBreakdown200ResponseSourcesInner and assigns it to the Sources field.
func (o *GetInboxSourceBreakdown200Response) SetSources(v []GetInboxSourceBreakdown200ResponseSourcesInner) {
	o.Sources = v
}

func (o GetInboxSourceBreakdown200Response) MarshalJSON() ([]byte, error) {
	toSerialize, err := o.ToMap()
	if err != nil {
		return []byte{}, err
	}
	return json.Marshal(toSerialize)
}

func (o GetInboxSourceBreakdown200Response) ToMap() (map[string]interface{}, error) {
	toSerialize := map[string]interface{}{}
	if !IsNil(o.Success) {
		toSerialize["success"] = o.Success
	}
	if !IsNil(o.From) {
		toSerialize["from"] = o.From
	}
	if o.To.IsSet() {
		toSerialize["to"] = o.To.Get()
	}
	if !IsNil(o.Sources) {
		toSerialize["sources"] = o.Sources
	}
	return toSerialize, nil
}

type NullableGetInboxSourceBreakdown200Response struct {
	value *GetInboxSourceBreakdown200Response
	isSet bool
}

func (v NullableGetInboxSourceBreakdown200Response) Get() *GetInboxSourceBreakdown200Response {
	return v.value
}

func (v *NullableGetInboxSourceBreakdown200Response) Set(val *GetInboxSourceBreakdown200Response) {
	v.value = val
	v.isSet = true
}

func (v NullableGetInboxSourceBreakdown200Response) IsSet() bool {
	return v.isSet
}

func (v *NullableGetInboxSourceBreakdown200Response) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableGetInboxSourceBreakdown200Response(val *GetInboxSourceBreakdown200Response) *NullableGetInboxSourceBreakdown200Response {
	return &NullableGetInboxSourceBreakdown200Response{value: val, isSet: true}
}

func (v NullableGetInboxSourceBreakdown200Response) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableGetInboxSourceBreakdown200Response) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}

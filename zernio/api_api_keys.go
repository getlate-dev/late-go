...
type APIKeysAPIVerifyCredentialRequest struct {
	ctx        context.Context
	ApiService *APIKeysAPIService
}

func (r APIKeysAPIVerifyCredentialRequest) Execute() (*VerifyCredential200Response, *http.Response, error) {
	return r.ApiService.VerifyCredentialExecute(r)
}
...
Checks whether the bearer credential on this request is valid, without reading any data. Accepts an API key or an OAuth access token. Intended for clients that must validate a credential before use (for example an MCP server verifying an incoming token) so they do not have to call a data endpoint to do it.
	...
	@param ctx context.Context - for authentication, logging, cancellation, deadlines, tracing, etc. Passed from http.Request or context.Background().
	@return APIKeysAPIVerifyCredentialRequest
*/
func (a *APIKeysAPIService) VerifyCredential(ctx context.Context) APIKeysAPIVerifyCredentialRequest {
	return APIKeysAPIVerifyCredentialRequest{
		ApiService: a,
		ctx:        ctx,
	}
...
func (a *APIKeysAPIService) VerifyCredentialExecute(r APIKeysAPIVerifyCredentialRequest) (*VerifyCredential200Response, *http.Response, error) {
	var (
		localVarHTTPMethod  = http.MethodGet
		localVarPostBody    interface{}
		formFiles           []formFile
		localVarReturnValue *VerifyCredential200Response
	)

	localBasePath, err := a.client.cfg.ServerURLWithContext(r.ctx, "APIKeysAPIService.VerifyCredential")
	if err != nil {
		return localVarReturnValue, nil, &GenericOpenAPIError{error: err.Error()}
	}

/*
Zernio API

API reference for Zernio. Authenticate with a Bearer API key. Base URL: https://zernio.com/api  Versioning and deprecation: all endpoints are versioned in the URL path (current version: /v1). Breaking changes only ship in a new path version; existing versions keep working. Deprecated operations are marked 'deprecated: true' in this spec and announced in the changelog (https://zernio.com/changelog) before removal.  Errors: every 4xx/5xx response is application/json with a machine-readable 'code' and a human-readable 'error' message (see the ErrorResponse schema).

API version: 1.0.4
Contact: support@zernio.com
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package zernio

import (
	"encoding/json"
)

// checks if the ListDiscordGuildMembers200Response type satisfies the MappedNullable interface at compile time
var _ MappedNullable = &ListDiscordGuildMembers200Response{}

// ListDiscordGuildMembers200Response struct for ListDiscordGuildMembers200Response
type ListDiscordGuildMembers200Response struct {
	Data       []DiscordGuildMember                          `json:"data,omitempty"`
	Pagination *ListDiscordGuildMembers200ResponsePagination `json:"pagination,omitempty"`
}

// NewListDiscordGuildMembers200Response instantiates a new ListDiscordGuildMembers200Response object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewListDiscordGuildMembers200Response() *ListDiscordGuildMembers200Response {
	this := ListDiscordGuildMembers200Response{}
	return &this
}

// NewListDiscordGuildMembers200ResponseWithDefaults instantiates a new ListDiscordGuildMembers200Response object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewListDiscordGuildMembers200ResponseWithDefaults() *ListDiscordGuildMembers200Response {
	this := ListDiscordGuildMembers200Response{}
	return &this
}

// GetData returns the Data field value if set, zero value otherwise.
func (o *ListDiscordGuildMembers200Response) GetData() []DiscordGuildMember {
	if o == nil || IsNil(o.Data) {
		var ret []DiscordGuildMember
		return ret
	}
	return o.Data
}

// GetDataOk returns a tuple with the Data field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *ListDiscordGuildMembers200Response) GetDataOk() ([]DiscordGuildMember, bool) {
	if o == nil || IsNil(o.Data) {
		return nil, false
	}
	return o.Data, true
}

// HasData returns a boolean if a field has been set.
func (o *ListDiscordGuildMembers200Response) HasData() bool {
	if o != nil && !IsNil(o.Data) {
		return true
	}

	return false
}

// SetData gets a reference to the given []DiscordGuildMember and assigns it to the Data field.
func (o *ListDiscordGuildMembers200Response) SetData(v []DiscordGuildMember) {
	o.Data = v
}

// GetPagination returns the Pagination field value if set, zero value otherwise.
func (o *ListDiscordGuildMembers200Response) GetPagination() ListDiscordGuildMembers200ResponsePagination {
	if o == nil || IsNil(o.Pagination) {
		var ret ListDiscordGuildMembers200ResponsePagination
		return ret
	}
	return *o.Pagination
}

// GetPaginationOk returns a tuple with the Pagination field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *ListDiscordGuildMembers200Response) GetPaginationOk() (*ListDiscordGuildMembers200ResponsePagination, bool) {
	if o == nil || IsNil(o.Pagination) {
		return nil, false
	}
	return o.Pagination, true
}

// HasPagination returns a boolean if a field has been set.
func (o *ListDiscordGuildMembers200Response) HasPagination() bool {
	if o != nil && !IsNil(o.Pagination) {
		return true
	}

	return false
}

// SetPagination gets a reference to the given ListDiscordGuildMembers200ResponsePagination and assigns it to the Pagination field.
func (o *ListDiscordGuildMembers200Response) SetPagination(v ListDiscordGuildMembers200ResponsePagination) {
	o.Pagination = &v
}

func (o ListDiscordGuildMembers200Response) MarshalJSON() ([]byte, error) {
	toSerialize, err := o.ToMap()
	if err != nil {
		return []byte{}, err
	}
	return json.Marshal(toSerialize)
}

func (o ListDiscordGuildMembers200Response) ToMap() (map[string]interface{}, error) {
	toSerialize := map[string]interface{}{}
	if !IsNil(o.Data) {
		toSerialize["data"] = o.Data
	}
	if !IsNil(o.Pagination) {
		toSerialize["pagination"] = o.Pagination
	}
	return toSerialize, nil
}

type NullableListDiscordGuildMembers200Response struct {
	value *ListDiscordGuildMembers200Response
	isSet bool
}

func (v NullableListDiscordGuildMembers200Response) Get() *ListDiscordGuildMembers200Response {
	return v.value
}

func (v *NullableListDiscordGuildMembers200Response) Set(val *ListDiscordGuildMembers200Response) {
	v.value = val
	v.isSet = true
}

func (v NullableListDiscordGuildMembers200Response) IsSet() bool {
	return v.isSet
}

func (v *NullableListDiscordGuildMembers200Response) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableListDiscordGuildMembers200Response(val *ListDiscordGuildMembers200Response) *NullableListDiscordGuildMembers200Response {
	return &NullableListDiscordGuildMembers200Response{value: val, isSet: true}
}

func (v NullableListDiscordGuildMembers200Response) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableListDiscordGuildMembers200Response) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}

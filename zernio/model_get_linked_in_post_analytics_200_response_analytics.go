/*
Zernio API

API reference for Zernio. Authenticate with a Bearer API key. Base URL: https://zernio.com/api  Versioning and deprecation: all endpoints are versioned in the URL path (current version: /v1). Breaking changes only ship in a new path version; existing versions keep working. Deprecated operations are marked 'deprecated: true' in this spec and announced in the changelog (https://zernio.com/changelog) before removal.  Errors: every 4xx/5xx response is application/json with a machine-readable 'code' and a human-readable 'error' message (see the ErrorResponse schema).

API version: 1.0.4
Contact: support@zernio.com
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package zernio

import (
	"encoding/json"
)

// checks if the GetLinkedInPostAnalytics200ResponseAnalytics type satisfies the MappedNullable interface at compile time
var _ MappedNullable = &GetLinkedInPostAnalytics200ResponseAnalytics{}

// GetLinkedInPostAnalytics200ResponseAnalytics struct for GetLinkedInPostAnalytics200ResponseAnalytics
type GetLinkedInPostAnalytics200ResponseAnalytics struct {
	// Times the post was shown
	Impressions *int32 `json:"impressions,omitempty"`
	// Unique members who saw the post
	Reach *int32 `json:"reach,omitempty"`
	// Reactions on the post
	Likes *int32 `json:"likes,omitempty"`
	// Comments on the post
	Comments *int32 `json:"comments,omitempty"`
	// Reshares of the post
	Shares *int32 `json:"shares,omitempty"`
	// Times the post was saved (personal accounts only; 0 for organization accounts)
	Saves *int32 `json:"saves,omitempty"`
	// Times the post was sent via LinkedIn messaging (personal accounts only; 0 for organization accounts)
	Sends *int32 `json:"sends,omitempty"`
	// Clicks on the post (organization accounts only)
	Clicks *int32 `json:"clicks,omitempty"`
	// Video views (video posts only)
	Views *int32 `json:"views,omitempty"`
	// Engagement rate, as a percentage rounded to 2 decimals: (likes + comments + shares + clicks + saves + sends) / impressions * 100. Unlike PostAnalytics.engagementRate on GET /v1/analytics, this one DOES count clicks and has no fallback denominator, so it is 0 whenever impressions is 0. For organization accounts the value is the rate LinkedIn returns, not one computed here.
	EngagementRate *float32 `json:"engagementRate,omitempty"`
}

// NewGetLinkedInPostAnalytics200ResponseAnalytics instantiates a new GetLinkedInPostAnalytics200ResponseAnalytics object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewGetLinkedInPostAnalytics200ResponseAnalytics() *GetLinkedInPostAnalytics200ResponseAnalytics {
	this := GetLinkedInPostAnalytics200ResponseAnalytics{}
	return &this
}

// NewGetLinkedInPostAnalytics200ResponseAnalyticsWithDefaults instantiates a new GetLinkedInPostAnalytics200ResponseAnalytics object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewGetLinkedInPostAnalytics200ResponseAnalyticsWithDefaults() *GetLinkedInPostAnalytics200ResponseAnalytics {
	this := GetLinkedInPostAnalytics200ResponseAnalytics{}
	return &this
}

// GetImpressions returns the Impressions field value if set, zero value otherwise.
func (o *GetLinkedInPostAnalytics200ResponseAnalytics) GetImpressions() int32 {
	if o == nil || IsNil(o.Impressions) {
		var ret int32
		return ret
	}
	return *o.Impressions
}

// GetImpressionsOk returns a tuple with the Impressions field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *GetLinkedInPostAnalytics200ResponseAnalytics) GetImpressionsOk() (*int32, bool) {
	if o == nil || IsNil(o.Impressions) {
		return nil, false
	}
	return o.Impressions, true
}

// HasImpressions returns a boolean if a field has been set.
func (o *GetLinkedInPostAnalytics200ResponseAnalytics) HasImpressions() bool {
	if o != nil && !IsNil(o.Impressions) {
		return true
	}

	return false
}

// SetImpressions gets a reference to the given int32 and assigns it to the Impressions field.
func (o *GetLinkedInPostAnalytics200ResponseAnalytics) SetImpressions(v int32) {
	o.Impressions = &v
}

// GetReach returns the Reach field value if set, zero value otherwise.
func (o *GetLinkedInPostAnalytics200ResponseAnalytics) GetReach() int32 {
	if o == nil || IsNil(o.Reach) {
		var ret int32
		return ret
	}
	return *o.Reach
}

// GetReachOk returns a tuple with the Reach field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *GetLinkedInPostAnalytics200ResponseAnalytics) GetReachOk() (*int32, bool) {
	if o == nil || IsNil(o.Reach) {
		return nil, false
	}
	return o.Reach, true
}

// HasReach returns a boolean if a field has been set.
func (o *GetLinkedInPostAnalytics200ResponseAnalytics) HasReach() bool {
	if o != nil && !IsNil(o.Reach) {
		return true
	}

	return false
}

// SetReach gets a reference to the given int32 and assigns it to the Reach field.
func (o *GetLinkedInPostAnalytics200ResponseAnalytics) SetReach(v int32) {
	o.Reach = &v
}

// GetLikes returns the Likes field value if set, zero value otherwise.
func (o *GetLinkedInPostAnalytics200ResponseAnalytics) GetLikes() int32 {
	if o == nil || IsNil(o.Likes) {
		var ret int32
		return ret
	}
	return *o.Likes
}

// GetLikesOk returns a tuple with the Likes field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *GetLinkedInPostAnalytics200ResponseAnalytics) GetLikesOk() (*int32, bool) {
	if o == nil || IsNil(o.Likes) {
		return nil, false
	}
	return o.Likes, true
}

// HasLikes returns a boolean if a field has been set.
func (o *GetLinkedInPostAnalytics200ResponseAnalytics) HasLikes() bool {
	if o != nil && !IsNil(o.Likes) {
		return true
	}

	return false
}

// SetLikes gets a reference to the given int32 and assigns it to the Likes field.
func (o *GetLinkedInPostAnalytics200ResponseAnalytics) SetLikes(v int32) {
	o.Likes = &v
}

// GetComments returns the Comments field value if set, zero value otherwise.
func (o *GetLinkedInPostAnalytics200ResponseAnalytics) GetComments() int32 {
	if o == nil || IsNil(o.Comments) {
		var ret int32
		return ret
	}
	return *o.Comments
}

// GetCommentsOk returns a tuple with the Comments field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *GetLinkedInPostAnalytics200ResponseAnalytics) GetCommentsOk() (*int32, bool) {
	if o == nil || IsNil(o.Comments) {
		return nil, false
	}
	return o.Comments, true
}

// HasComments returns a boolean if a field has been set.
func (o *GetLinkedInPostAnalytics200ResponseAnalytics) HasComments() bool {
	if o != nil && !IsNil(o.Comments) {
		return true
	}

	return false
}

// SetComments gets a reference to the given int32 and assigns it to the Comments field.
func (o *GetLinkedInPostAnalytics200ResponseAnalytics) SetComments(v int32) {
	o.Comments = &v
}

// GetShares returns the Shares field value if set, zero value otherwise.
func (o *GetLinkedInPostAnalytics200ResponseAnalytics) GetShares() int32 {
	if o == nil || IsNil(o.Shares) {
		var ret int32
		return ret
	}
	return *o.Shares
}

// GetSharesOk returns a tuple with the Shares field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *GetLinkedInPostAnalytics200ResponseAnalytics) GetSharesOk() (*int32, bool) {
	if o == nil || IsNil(o.Shares) {
		return nil, false
	}
	return o.Shares, true
}

// HasShares returns a boolean if a field has been set.
func (o *GetLinkedInPostAnalytics200ResponseAnalytics) HasShares() bool {
	if o != nil && !IsNil(o.Shares) {
		return true
	}

	return false
}

// SetShares gets a reference to the given int32 and assigns it to the Shares field.
func (o *GetLinkedInPostAnalytics200ResponseAnalytics) SetShares(v int32) {
	o.Shares = &v
}

// GetSaves returns the Saves field value if set, zero value otherwise.
func (o *GetLinkedInPostAnalytics200ResponseAnalytics) GetSaves() int32 {
	if o == nil || IsNil(o.Saves) {
		var ret int32
		return ret
	}
	return *o.Saves
}

// GetSavesOk returns a tuple with the Saves field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *GetLinkedInPostAnalytics200ResponseAnalytics) GetSavesOk() (*int32, bool) {
	if o == nil || IsNil(o.Saves) {
		return nil, false
	}
	return o.Saves, true
}

// HasSaves returns a boolean if a field has been set.
func (o *GetLinkedInPostAnalytics200ResponseAnalytics) HasSaves() bool {
	if o != nil && !IsNil(o.Saves) {
		return true
	}

	return false
}

// SetSaves gets a reference to the given int32 and assigns it to the Saves field.
func (o *GetLinkedInPostAnalytics200ResponseAnalytics) SetSaves(v int32) {
	o.Saves = &v
}

// GetSends returns the Sends field value if set, zero value otherwise.
func (o *GetLinkedInPostAnalytics200ResponseAnalytics) GetSends() int32 {
	if o == nil || IsNil(o.Sends) {
		var ret int32
		return ret
	}
	return *o.Sends
}

// GetSendsOk returns a tuple with the Sends field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *GetLinkedInPostAnalytics200ResponseAnalytics) GetSendsOk() (*int32, bool) {
	if o == nil || IsNil(o.Sends) {
		return nil, false
	}
	return o.Sends, true
}

// HasSends returns a boolean if a field has been set.
func (o *GetLinkedInPostAnalytics200ResponseAnalytics) HasSends() bool {
	if o != nil && !IsNil(o.Sends) {
		return true
	}

	return false
}

// SetSends gets a reference to the given int32 and assigns it to the Sends field.
func (o *GetLinkedInPostAnalytics200ResponseAnalytics) SetSends(v int32) {
	o.Sends = &v
}

// GetClicks returns the Clicks field value if set, zero value otherwise.
func (o *GetLinkedInPostAnalytics200ResponseAnalytics) GetClicks() int32 {
	if o == nil || IsNil(o.Clicks) {
		var ret int32
		return ret
	}
	return *o.Clicks
}

// GetClicksOk returns a tuple with the Clicks field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *GetLinkedInPostAnalytics200ResponseAnalytics) GetClicksOk() (*int32, bool) {
	if o == nil || IsNil(o.Clicks) {
		return nil, false
	}
	return o.Clicks, true
}

// HasClicks returns a boolean if a field has been set.
func (o *GetLinkedInPostAnalytics200ResponseAnalytics) HasClicks() bool {
	if o != nil && !IsNil(o.Clicks) {
		return true
	}

	return false
}

// SetClicks gets a reference to the given int32 and assigns it to the Clicks field.
func (o *GetLinkedInPostAnalytics200ResponseAnalytics) SetClicks(v int32) {
	o.Clicks = &v
}

// GetViews returns the Views field value if set, zero value otherwise.
func (o *GetLinkedInPostAnalytics200ResponseAnalytics) GetViews() int32 {
	if o == nil || IsNil(o.Views) {
		var ret int32
		return ret
	}
	return *o.Views
}

// GetViewsOk returns a tuple with the Views field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *GetLinkedInPostAnalytics200ResponseAnalytics) GetViewsOk() (*int32, bool) {
	if o == nil || IsNil(o.Views) {
		return nil, false
	}
	return o.Views, true
}

// HasViews returns a boolean if a field has been set.
func (o *GetLinkedInPostAnalytics200ResponseAnalytics) HasViews() bool {
	if o != nil && !IsNil(o.Views) {
		return true
	}

	return false
}

// SetViews gets a reference to the given int32 and assigns it to the Views field.
func (o *GetLinkedInPostAnalytics200ResponseAnalytics) SetViews(v int32) {
	o.Views = &v
}

// GetEngagementRate returns the EngagementRate field value if set, zero value otherwise.
func (o *GetLinkedInPostAnalytics200ResponseAnalytics) GetEngagementRate() float32 {
	if o == nil || IsNil(o.EngagementRate) {
		var ret float32
		return ret
	}
	return *o.EngagementRate
}

// GetEngagementRateOk returns a tuple with the EngagementRate field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *GetLinkedInPostAnalytics200ResponseAnalytics) GetEngagementRateOk() (*float32, bool) {
	if o == nil || IsNil(o.EngagementRate) {
		return nil, false
	}
	return o.EngagementRate, true
}

// HasEngagementRate returns a boolean if a field has been set.
func (o *GetLinkedInPostAnalytics200ResponseAnalytics) HasEngagementRate() bool {
	if o != nil && !IsNil(o.EngagementRate) {
		return true
	}

	return false
}

// SetEngagementRate gets a reference to the given float32 and assigns it to the EngagementRate field.
func (o *GetLinkedInPostAnalytics200ResponseAnalytics) SetEngagementRate(v float32) {
	o.EngagementRate = &v
}

func (o GetLinkedInPostAnalytics200ResponseAnalytics) MarshalJSON() ([]byte, error) {
	toSerialize, err := o.ToMap()
	if err != nil {
		return []byte{}, err
	}
	return json.Marshal(toSerialize)
}

func (o GetLinkedInPostAnalytics200ResponseAnalytics) ToMap() (map[string]interface{}, error) {
	toSerialize := map[string]interface{}{}
	if !IsNil(o.Impressions) {
		toSerialize["impressions"] = o.Impressions
	}
	if !IsNil(o.Reach) {
		toSerialize["reach"] = o.Reach
	}
	if !IsNil(o.Likes) {
		toSerialize["likes"] = o.Likes
	}
	if !IsNil(o.Comments) {
		toSerialize["comments"] = o.Comments
	}
	if !IsNil(o.Shares) {
		toSerialize["shares"] = o.Shares
	}
	if !IsNil(o.Saves) {
		toSerialize["saves"] = o.Saves
	}
	if !IsNil(o.Sends) {
		toSerialize["sends"] = o.Sends
	}
	if !IsNil(o.Clicks) {
		toSerialize["clicks"] = o.Clicks
	}
	if !IsNil(o.Views) {
		toSerialize["views"] = o.Views
	}
	if !IsNil(o.EngagementRate) {
		toSerialize["engagementRate"] = o.EngagementRate
	}
	return toSerialize, nil
}

type NullableGetLinkedInPostAnalytics200ResponseAnalytics struct {
	value *GetLinkedInPostAnalytics200ResponseAnalytics
	isSet bool
}

func (v NullableGetLinkedInPostAnalytics200ResponseAnalytics) Get() *GetLinkedInPostAnalytics200ResponseAnalytics {
	return v.value
}

func (v *NullableGetLinkedInPostAnalytics200ResponseAnalytics) Set(val *GetLinkedInPostAnalytics200ResponseAnalytics) {
	v.value = val
	v.isSet = true
}

func (v NullableGetLinkedInPostAnalytics200ResponseAnalytics) IsSet() bool {
	return v.isSet
}

func (v *NullableGetLinkedInPostAnalytics200ResponseAnalytics) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableGetLinkedInPostAnalytics200ResponseAnalytics(val *GetLinkedInPostAnalytics200ResponseAnalytics) *NullableGetLinkedInPostAnalytics200ResponseAnalytics {
	return &NullableGetLinkedInPostAnalytics200ResponseAnalytics{value: val, isSet: true}
}

func (v NullableGetLinkedInPostAnalytics200ResponseAnalytics) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableGetLinkedInPostAnalytics200ResponseAnalytics) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}
